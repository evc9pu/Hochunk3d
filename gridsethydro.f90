subroutine gridsethydro(iter,i1)
...
!
  ...
  integer*4 :: nrg1,ntg1,npg1,ndg1

  integer :: peelid
  character :: cmsgnm*70

  call diskdat_section('gridsethydro')
...
if (rddust(2).gt.rmaxd(2)) then
  ...
  print*,'before opening file'
   open(unit=29,file='densarr.unf',status='unknown',form='unformatted')
  print*,'after opening'
  print*,'read a line'
   read(29)
  print*,'done'
!  read(29) nrg1,ntg1,npg1,ndg1
!   stop
   read(29) (rarr(ir),ir=1,nrg)
  print*,rarr
!   stop 
  read(29) (thetarr(it),it=1,ntg)
!  print*,thetarr
   read(29) (phiarr(ip),ip=1,npg)
!  print*,phiarr
  read(29) ((((densarr(ir,it,ip,id),ir=1,nrg),it=1,ntg),ip=1,npg),id=1,2)
   close(29) 

!  nttmp=ntg/2
!  do ir=1,nrg
!  print*,'densarr',densarr(ir,nttmp,1,1)
!  enddo
!  print*,'nttmp',nttmp

!  print*,nrg1,ntg1,npg1,ndg1
!  print*,nrg,ntg,npg,ndg
!  stop

  ! check before roundoff error fix
  print*,'rddustmin,rarr(2)',rddustmin,rarr(2)
  print*,'rarr(1),rmin',rarr(1),rmin
  print*,'rarr(nrg),rmax',rarr(nrg),rmax
  print*,'rminsub,rmine,rmind',rminsub,rmine,rmind

  ! fix roundoff errors
  if (rddust(1).lt.rarr(2)) rddust(1)=rarr(2)
  if (rddust(2).lt.rarr(2)) rddust(2)=rarr(2)
!  rddust=rarr(2)
  rarr(1)=rmin
  rarr(nrg)=rmax
  if (rmaxd(1).gt.rarr(nrg)) rmaxd(1)=rarr(nrg)
  if (rmaxd(2).gt.rarr(nrg)) rmaxd(2)=rarr(nrg)
  if (rmine.lt.rarr(2)) rmine=rarr(2)
  ! irminsub=1 in this version of code.   now I forget why a few weeks later
  rminsub=rddust(1)/rmind_in(1)    !I think we're just recalculating for roundoff.  originally, we set rmind=rminsub*rmind_in
  rmine=rminsub*rmine_in
  rmind=rminsub*rmind_in
  rddust=rminsub*rmind_in
  print*,'rminsub,rmine,rmind',rminsub,rmine,rmind
  ...
  print*,'thetarr(1),thetarr(ntg),phiarr(1),phiarr(npg)',thetarr(1),thetarr(ntg),phiarr(1),phiarr(npg)
  thetarr(nrg)=pi 
  thetarr(1)=0.d0
  phiarr(1)=0.d0
  phiarr(npg)=r2p
...
rhogap0(3:4)=0.d0
              ...
              do it=1,ntg-1
                 ...
                 do ip=1,npg-1 !3-D atmosphere
                   ...
                   ! call densdisk(rad,sint,cost,phi,densd,id)
                    ...
                    tmpmass(id)=tmpmass(id)+densarr(ir,it,ip,id)*vol
                 end do
              end do
           end do
        end if
  ...
  print*,'massdisk (all grains)',massdisk
  print*,'mass of each disk',tmpmass
...
!                 densarr(ir1,it1,ip1,id)=densd*diskscl(id) ! densd already includes fmassd scale factor in rho0
           ...
           !      massdisk=massdisk+densd*diskscl(id)*vcell(ir1,it1,ip1)
                 massdisk=massdisk+densarr(ir1,it1,ip1,id)*vcell(ir1,it1,ip1)   !hydro
                 tmpmass(id)=tmpmass(id)+densarr(ir1,it1,ip1,id)*vcell(ir1,it1,ip1)  !hydro
        !         tmpmass(id)=tmpmass(id)+densd*diskscl(id)*vcell(ir1,it1,ip1)
                  sigarr(ir1,ip1,id)=sigarr(ir1,ip1,id)+abs(densarr(ir1,it1,ip1,id)*rad*dcost*rmincgs)  !rad*dcost=dz
...
! calculate calculate scale factor for sigarr (theta integration); also do an analytic comparison first (along z)
 ...
 enddo
 print*,'sigpwtot,sigtot, should agree with input mass',sigpwtot/msun,sigtot/msun,massd

 sigscale=massd/sigtot*msun
 print*,'sigscale should be 1 if input disk mass is correct',sigscale
 print*,'for hydro reading in, setting sigscale = 1'
 sigscale=1.d0
