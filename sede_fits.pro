pro sede_fits,dirm,rstar,tstar,iplotstar,lumscale,intit,d,napin,$
...
print,'integrated luminosity, no foreground extinction applied'
  ...
  if flux eq 't' then print,'integrated thermal luminosity (lsun)',sum/float(npeel)/fnorm
  if flux eq 'd' then print,'integrated direct stellar luminosity (lsun)',sum/float(npeel)/fnorm
  if flux eq 's' then print,'integrated scattered luminosity (lsun)',sum/float(npeel)/fnorm
...
for it=0,npeel-1 do begin
  ...
  f5t(*,it,nap)=f5t(*,it,nap)*extinct
endfor

;convert nuhnu and f4 to Jy; units are currently in lamflam=nufnu (erg...)
;first, divide by frequency to get erg/cm2/s/hz
;then multiply by 1.e23 to convert to Jy
;fnu=lamflam/nu
;c is in microns/s
;x4 is in microns
if Jy eq 1 then begin
  for it=0,npeel-1 do begin
    f5(*,it,nap)=f5(*,it,nap)*1.e23/c*x5
    f5s(*,it,nap)=f5s(*,it,nap)*1.e23/c*x5
    f5d(*,it,nap)=f5d(*,it,nap)*1.e23/c*x5
    f5t(*,it,nap)=f5t(*,it,nap)*1.e23/c*x5
  endfor
  nuhnu=nuhnu*1.e23/c*lama
 ; print,'max f5',max(f5)
endif

print,'max f5',max(f5)

;maxm=max(nuhnu,/nan)*3.
maxm=max(f5,/nan)*2.
;maxm=1.e-6
minm=maxm*1.e-5
...
;flux at 70, 160 um
for it=0,npeel-1 do begin
fhi=f5(0:nfreq-1,it,nap)
fhi6=f6(0:nfreq-1,it,nap)
;fhism=smooth(fhi,3)
;fhi=fhism
;help,fhi
;help,x5
...
;print,x5(i),fhi(i)
;endfor
;f3_6=interpol(fhi,x5,3.33)
;f3_6=interpol(fhi,x5,3.6)
f4_5=interpol(fhi,x5,4.5)
f4_5d=interpol(fhi6,x5,4.5)
;print,'f4_5,f4_5d',f4_5,f4_5d
;print,'f4_5-f4_5d/f4_5d',(f4_5-f4_5d)/f4_5d
;f5_8=interpol(fhi,x5,5.8)
;f70=interpol(fhi,x5,70.)
;f160=interpol(fhi,x5,160.)
